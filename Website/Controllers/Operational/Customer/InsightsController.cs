...
namespace MyVoltage.Controllers.Operational
{
    ...
    public class InsightsController : Controller
    {
        ...
        public InsightsController(IMemoryCache cache, UserManager<ApplicationUser> userManager, DbContextOptions<Data.MyVoltageDbContext> options, OperationalProvider operationalProvider, IHttpContextAccessor contextAccessor, DbContextOptions<MyVoltageApi.Data.MyVoltageApiDbContext> APIoptions, IConfiguration configuration)
        {
        ...
        [HttpGet]
        [Route("/operational/Customer/Customer_Insights")]
        public async Task<IActionResult> Insights()
        {
            ...
            return View("~/Views/Operational/Customer/Insights/Insights.cshtml", model);
        }

        [HttpGet]
        [Route("/operational/Customer/Customer_Insights/AnnualCostTotal")]
        public async Task<IActionResult> AnnualCostTotal()
        {
            ...
            if (!string.IsNullOrEmpty(_operationalProvider.CustomerNumber))
            {
                ...
                foreach (var product in products)
                {
                    ...
                    while (current <= endDate)
                    {
                        ...
                        if (entry != null)
                        {
                            if (_operationalProvider.ShowCostInclVAT)
            ...
            return PartialView("~/Views/Operational/Customer/Insights/AnnualCostTotal.cshtml", model);
        }

        [HttpGet]
        [Route("/operational/Customer/Customer_Insights/ProductsDailyCost")]
        public async Task<IActionResult> ProductsDailyCost()
        {
            ...
            if (!string.IsNullOrEmpty(_operationalProvider.CustomerNumber))
            {
                ...
                foreach (var product in products)
                {
                    ...
                    var resourceLedgersForProduct = (from p in db.SkybillResourceLedgerEntries
                                                     ...
                                                     && p.CompanyID == _operationalProvider.CompanyID
                                                     && p.Source_No == _operationalProvider.CustomerNumber
                                                     select new
                                                     {
                                                         p.Posting_Date,
                                                         p.Total_Price,
                    ...
                    while (current <= endDate)
                    {
                        ...
                        if (resourceLedgerEntries.Count > 0)
                        {
                            ...
                            {
                                Amount = _operationalProvider.ShowCostInclVAT ? (resourceLedgerEntries.Select(p => p.Amount).Sum() * 1.15m) : resourceLedgerEntries.Select(p => p.Amount).Sum(),
                                Date = current,
                    ...
                    while (current <= endDateMonthly)
                    {
                        ...
                        if (entry != null)
                        {
                            ProductsDailyCostModel.ProductItem.BillingItem item = new ProductsDailyCostModel.ProductItem.BillingItem()
                            {
                                Amount = _operationalProvider.ShowCostInclVAT ? (entry.Amount * 1.15m) : entry.Amount,
                                Date = current,
            ...
            return PartialView("~/Views/Operational/Customer/Insights/ProductsDailyCost.cshtml", model);
        }

        [HttpGet]
        [Route("/operational/Customer/Customer_Insights/CustomerBilling")]
        public async Task<IActionResult> CustomerBilling()
        {
            ...
            if (!string.IsNullOrEmpty(_operationalProvider.CustomerNumber))
            {
                ...
                MyVoltage.Services.Operational.OperationalBillingProvider operationalBillingProvider = new Services.Operational.OperationalBillingProvider(_cache, _operationalProvider);

                var allLedgers = operationalBillingProvider.GetLedgerEntriesByCustomer();
            ...
            return PartialView("~/Views/Operational/Customer/Insights/CustomerBilling.cshtml", model);
        }
