...
using MyVoltage.Models.OperationalModels.Shared.SharedCustomerModels;
...
namespace MyVoltage.Controllers.Operational.Shared
{
    ...
    public class SharedCustomerController : Controller
    {
        ...
        [HttpGet]
        [Route("/operational/shared/customer/customerdetails")]
        public async Task<IActionResult> CustomerDetails()
        {
            ...
            if (!string.IsNullOrEmpty(_operationalProvider.CustomerNumber))
            {
                ...
                MyVoltage.Api.SkyBill.SkyBillApiClient skyBillApiClient = new MyVoltage.Api.SkyBill.SkyBillApiClient(company.Name, _cache, _operationalProvider.UseAzureSkybill);
                var skybillApiCustomer = skyBillApiClient.GetCustomer(_operationalProvider.CustomerNumber);
            ...
            return PartialView("~/Views/Operational/Shared/Customer/CustomerDetails.cshtml", model);

        }

        [HttpGet]
        [Route("/operational/shared/customer/customerusage")]
        public async Task<IActionResult> CustomerUsage()
        {
            Models.OperationalModels.Shared.SharedCustomerModels.CustomerUsageModel model = new Models.OperationalModels.Shared.SharedCustomerModels.CustomerUsageModel()
            {
            ...
            if (!string.IsNullOrEmpty(_operationalProvider.CustomerNumber))
            {
                ...
                model.CustomerBalance = (decimal)skyBillApiClient.GetCustomer(_operationalProvider.CustomerNumber).Balance_LCY * multiplier;

                List<Tuple<string, decimal, DateTime>> payments = new List<Tuple<string, decimal, DateTime>>();

                UniPin latestUnipin = null;
                foreach (var meter in skyBillApiClient.GetMetersByCustomer(_operationalProvider.CustomerNumber))
                {
                ...
                var latestDirectDeposit = db.NetcashManualPayments.Where(p => p.CustomerNo == _operationalProvider.CustomerNumber).OrderByDescending(p => p.DateCreated).FirstOrDefault();
            ...
            return PartialView("~/Views/Operational/Shared/Customer/CustomerUsage.cshtml", model);

        }

        [HttpGet]
        [Route("/operational/shared/customer/customermeterlist")]
        public async Task<IActionResult> CustomerMeterList()
        {
            ...
            return PartialView("~/Views/Operational/Shared/Customer/CustomerMeterList.cshtml", model);

        }

        [HttpGet]
        [Route("/operational/shared/customer/customerGraphsMonthly")]
        public async Task<IActionResult> CustomerGraphsMonthly()
        {
            CustomerMeterListModel model = new CustomerMeterListModel()
            {
                Devices = new List<CustomerMeterListModel.MeterItem>()
            ...
            return PartialView("~/Views/Operational/Shared/Customer/CustomerGraphsMonthly.cshtml", model);

        }

        [HttpGet]
        [Route("/operational/shared/customer/customeroccupancystatus")]
        public async Task<IActionResult> CustomerOccupancyStatus()
        {
            CustomerOccupancyStatusModel model = new CustomerOccupancyStatusModel()
            {
            };
            ...
            {
                MyVoltageDbContext db = new MyVoltageDbContext(_options);
                var latestEntry = (from p in db.Log_BillingControlReport_OccupancyVerifications
                ...
                {
                    bool isValid = false;
                    var opProv = db.OperationalProfiles.Where(p => p.UserID == latestEntry.UserID).FirstOrDefault();

                    model = new CustomerOccupancyStatusModel()
                    {
                        CompanyID = latestEntry.CompanyID,
                        CreateDate = latestEntry.CreateDate,
                        CustomerNo = latestEntry.CustomerNo,
                        ID = latestEntry.ID,
                        Occupancy = latestEntry.Occupancy,
                        UserID = latestEntry.UserID,
                        IsValid = isValid,
                        FullName = opProv != null ? opProv.FirstName + " " + opProv.LastName : _userManager.FindByIdAsync(latestEntry.UserID).Result.UserName,
                    };
                }

            }

            return PartialView("~/Views/Operational/Shared/Customer/CustomerOccupancyStatus.cshtml", model);
        }
        [HttpGet]

        [Route("/operational/shared/customer/customerproductsresourceledgersformonth/{year?}/{month?}")]
        public async Task<IActionResult> CustomerProductsResourceLedgersForMonth(int? year, int? month)
        {
            ...
            if (!string.IsNullOrEmpty(_operationalProvider.CustomerNumber))
            {
                ...
                var resourceEntriesPerProductForCustomer = (from p in db.Report_ProductsResourceLedgerCustomerMonthlies
                                                            ...
                                                            && p.CustomerNo == _operationalProvider.CustomerNumber
            ...
            return PartialView("~/Views/Operational/Shared/Customer/CustomerProductsResourceLedgersForMonth.cshtml", model);
