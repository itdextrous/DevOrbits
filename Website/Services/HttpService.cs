...
namespace MyVoltage.Services
{
    public class HttpService : IHttpService
    {
        ...
        private readonly IWrikeService _wrikeService;
        public string TokenExpiryTime { get; set; }
        public Uri options { get; private set; }
        ...
        public HttpService(IConfiguration configuration,
            ...
            IWrikeService wrikeService)
        {
            CLIENT_ID = configuration.GetSection("Token:ClientId").Value;
            CLIENT_SECRET = configuration.GetSection("Token:ClientSecret").Value;
            AccessToken = configuration.GetSection("AccessToken:Bearer").Value;
            _externalServiceModel = externalServiceModel;
            _options = options;
            _client = client;
            _userManager = userManager;
            _wrikeService = wrikeService;
        }
        ...
        //-----------------Create Tasks From MyVoltage----------------------------------
        public async Task<string> PostAsync(WrikeViewModel model)
        {
            ...
            using (var client = new HttpClient())
            {
                try
                {
                    ...
                    var mainCompanyFolderId = await GetMainFolderIdByName("Company Tasks and Flags");
                    string companyId;
                    string childFolderId;
                    string taskLocationId;
                    var company = db.Companies.Where(x => x.Name == model.Company).FirstOrDefault();
                    if (company != null)
                    {
                        companyId = model.Company;
                        childFolderId = await GetChildFolderById(mainCompanyFolderId, companyId);
                        if (childFolderId != null)
                        {
                        taskLocationId = await GetChildFolderById(childFolderId, "Tasks");
                        }
                        else
                        {
                            companyId = await GetFolderById(mainCompanyFolderId, "Company");
                            taskLocationId = companyId;
                        }
                    }
                    else
                    {
                    ...
                    var mainWorkflowFolderId = await GetMainFolderIdByName("Workflow Tasks and Flags");
                    var childFolderIdWorkflow = await GetChildFolderById(mainWorkflowFolderId, model.WorkflowGroupName);
                    var workflowLocationId = await GetChildFolderById(childFolderIdWorkflow, model.Title);

                    var restClient = new RestSharp.RestClient($"{_externalServiceModel.Value.WrikeApi}/folders/{taskLocationId}/tasks");
                    ...
                    var startDate = model.DateStarted != null ? model.DateStarted.Value.ToString("yyyy-MM-ddTHH:mm:ss") : model.DateCreated.ToString("yyyy-MM-ddTHH:mm:ss");
                    DateTime dueDate = (DateTime)model.DueDate;
                    var due_date = dueDate.ToString("yyyy-MM-ddTHH:mm:ss");
                    var date = $"'start':{startDate},'due':{due_date}";
                    dynamic dynamicDate = new System.Dynamic.ExpandoObject();
                    dynamicDate.start = startDate;
                    dynamicDate.due = due_date;
                    var taskId = model.ID.ToString();
                    request.AddHeader("Authorization", "Bearer " + AccessToken);
                    request.AddHeader("Content-Type", "application/x-www-form-urlencoded");
                    request.AddHeader("Cookie", "isWhitelabel=false; wrikeLocale=en");
                    request.AddParameter("description", model.Description);
                    request.AddParameter("dates", $"{JsonConvert.SerializeObject(dynamicDate)}");
                    if (workflowLocationId != null)
                    {
                        request.AddParameter("parents", "['" + workflowLocationId + "']");
                    }
                    
                    request.AddParameter("effortAllocation", " {\"mode\": \"Basic\", \"totalEffort\":'" + (model.DefaultMinPlanned != null ? model.DefaultMinPlanned : 0) + "'}");
                    request.AddParameter("customFields", "[{\"id\":\"IEAFS2NXJUADYQBX\",\"value\":'" + model.Priority + "'}," +
                        "{\"id\":\"IEAFS2NXJUADYI5V\",\"value\":'" + currentDate + "'}," +
                        "{\"id\":\"IEAFS2NXJUADYQAR\",\"value\":'" + model.ReportingToUser + "'}," +
                        "{\"id\":\"IEAFS2NXJUADYP7Z\",\"value\":'" + model.Status + "'}," +
                    ...
                    if (contactId != null)
                    {
                        request.AddParameter("responsibles", "['" + contactId + "']");
                    }
                    IRestResponse response = restClient.Execute(request);
                    Console.WriteLine(response.Content);

                    var result = Newtonsoft.Json.JsonConvert.DeserializeObject<WrikeDetails>(response.Content);
        ...
        public async Task<string> UpdateAsync(WrikeUpdateViewModel model)
        {
            ...
            using (var client = new HttpClient())
            {
                try
                {
                    ...
                    var currentDate = DateTime.Now.ToString("yyyy-MM-dd");
                    var startDate = model.DateStarted != null ? model.DateStarted?.ToString("yyyy-MM-ddTHH:mm:ss") : model.DateCreated.ToString("yyyy-MM-ddTHH:mm:ss");
                    var dueDate = model.DueDate?.ToString("yyyy-MM-ddTHH:mm:ss");
                    var date = $"'start':{startDate},'due':{dueDate}";
                    dynamic dynamicDate = new System.Dynamic.ExpandoObject();
                    dynamicDate.start = startDate;
        ...
        {
            if (string.IsNullOrEmpty(AccessToken) && string.IsNullOrEmpty(TokenExpiryTime))
                return true;

            if (Convert.ToDateTime(TokenExpiryTime).Subtract(DateTime.UtcNow).TotalSeconds < 3600 && Convert.ToDateTime(TokenExpiryTime) != DateTime.MinValue)
                return true;
            else
                return false;
        }
        public async Task<AuthTokenResponse> GetAccessToken()
        {
            ...
            //var apiUrl = "https://login.wrike.com/oauth2/token";
            //Token tokenCredentials = new Token
            //{
            //    client_id = CLIENT_ID,
            //    client_secret = CLIENT_SECRET,
            //    grant_type = "authorization_code",
            //    code = "ZuccrkYT7Rt6HezGZDP37EYKkialoZ8skvxxpBzdIgeyCmu7WkR7inAPfF0N6R0V_eu-N"
            //};
            //var request = JsonConvert.SerializeObject(tokenCredentials);
            //using (var client = new HttpClient())
            //{
            //    HttpContent requestObj = new StringContent(request, UnicodeEncoding.UTF8, "application/x-www-form-urlencode");

            //    var result = await client.PostAsync(apiUrl, requestObj);
            //    var data = await result.Content.ReadAsStringAsync();
            //    if (result.StatusCode == HttpStatusCode.OK)
            //    {
            //        tokenResponse = JsonConvert.DeserializeObject<TokenResponse>(data);
            //        AccessToken = tokenResponse.Access_Token;
            //        TokenExpiryTime = DateTime.UtcNow.AddSeconds(tokenResponse.Expires_In).ToString();
            //    }
            //    else
            ...
            var client1 = new RestSharp.RestClient("https://login.wrike.com/oauth2/token");
            client1.Timeout = -1;
            var request = new RestRequest(Method.POST);
            request.AddHeader("Content-Type", "application/x-www-form-urlencoded");
            request.AddHeader("Cookie", "isWhitelabel=false; wrikeLocale=en; JSESSIONID=E9F62695E06C198295C7611DEE8615CD; wrikeForceSSO=false;");
            request.AddParameter("client_id", CLIENT_ID);
            request.AddParameter("client_secret", CLIENT_SECRET);
            request.AddParameter("grant_type", "authorization_code");
            request.AddParameter("code", "yIv8mYDC0SiN43hO7cdepfsI93fCWCM3aCRIG8tQYnUOOEboorAUHr3TFe49PVsw_eu-N");
            IRestResponse response = client1.Execute(request);
            if (response.StatusCode == HttpStatusCode.OK)
            {
                tokenResponse = JsonConvert.DeserializeObject<AuthTokenResponse>(response.Content);
                AccessToken = tokenResponse.access_token;
                TokenExpiryTime = DateTime.UtcNow.AddSeconds(tokenResponse.expires_in).ToString();
            }
        ...
        public async Task<bool> UpdateCustomFieldToDB(string taskId)
        {
            try
            {
                ...
                if (taskDataFromDatabase != null)
                {
                    ...
                    string wrikeId = taskDataFromDatabase.WrikeID;
                    wrikeId = await UpdateWorkflowGroup(taskDataFromDatabase);
                    return true;
                }
                return false;
            }
        ...
        public async Task<string> UpdateWorkflowGroup(A08_Task model)
        {
            var db = new MyVoltageDbContext(_options);
            var responseString = string.Empty;
            using (var client = new HttpClient())
            ...
            try
            {
                ...
                if (newTaskData != null)
                {
                    ...
                    if (companyId != null)
                    {
                        ...
                        db.SaveChanges();
                        string wrikeId = wrikeHookModel.TaskId;
                        wrikeId = await UpdateWrike(a08_Task);
                    }
                }
                return true;
            }
        ...
        //------------------Update task details in wrike-----------------
        public async Task<string> UpdateWrike(A08_Task model)
        {
            var db = new MyVoltageDbContext(_options);
            var responseString = string.Empty;
            var TaskPriority = await GetTaskPriority((int)model.PriorityID);
            var TaskDefaultMinPlanned = await GetDefaultMinPlanned(model.TaskTypeID);
            using (var client = new HttpClient())
            {
                try
                {
                    ...
                    request.AddHeader("Authorization", "Bearer " + AccessToken);
                    request.AddHeader("Content-Type", "application/x-www-form-urlencoded");
                    request.AddParameter("effortAllocation", " {\"mode\": \"Basic\", \"totalEffort\":'" + (TaskDefaultMinPlanned != null ? TaskDefaultMinPlanned : "") + "'}");
                    request.AddParameter("customFields", "[" +
                        "{\"id\":\"IEAFS2NXJUADYQBX\",\"value\":'" + TaskPriority + "'}," +
                        "{\"id\":\"IEAFS2NXJUADYI5V\",\"value\":'" + currentDate + "'}," +
                        "{\"id\":\"IEAFS2NXJUADYQAR\",\"value\":'" + model.ReportingToUserID + "'},"
                        //+
                        //"{\"id\":\"IEAFS2NXJUADYTDB\",\"value\":'" + model.WorkflowGroupName + "'}," +
                        //"{\"id\":\"IEAFS2NXJUADYTDC\",\"value\":'" + model.BusinessPillarName + "'}," +
                        //"{\"id\":\"IEAFS2NXJUADYTDD\",\"value\":'" + model.BusinessDepartmentName + "'}]"
        ...
        public async Task<string> GetTaskPriority(int PriorityID)
        {
            try
            {
                var db = new MyVoltageDbContext(_options);
                var taskTypes = db.SiteAdmin_Priorities.Where(x => x.ID == PriorityID).FirstOrDefault();
                var priority = taskTypes != null ? taskTypes.PriorityName : null;
                return priority;
            }
            catch (Exception ex)
            {
        ...
        //---------------Get Default Minutes Planned value-------------------
        public async Task<string> GetDefaultMinPlanned(int TaskTypeID)
        {
            try
            {
                var db = new MyVoltageDbContext(_options);
                var taskTypes = db.A08_Task_Types.Where(x => x.ID == TaskTypeID).FirstOrDefault();
                var defaultMinPlanned = taskTypes != null ? taskTypes.DefautlMinPlanned : null;
                return defaultMinPlanned.ToString();
            }
            catch (Exception ex)
            {
                throw ex;
