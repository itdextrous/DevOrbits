...
namespace MyVoltage.Services.Operational
{
    public class OperationalBillingProvider
    {
        ...
        public OperationalBillingProvider(IMemoryCache cache, OperationalProvider operationalProvider)
        {
            _companyName = operationalProvider.Companies.Where(p => p.CompanyID == operationalProvider.CompanyID).SingleOrDefault().Name;
            _operationalProvider = operationalProvider;
            _cache = cache;
            string customerNo = operationalProvider.CustomerNumber;

            if (!string.IsNullOrEmpty(customerNo))
            {
                //string key = customerNo + "_" + (int)operationalProvider.AccountTypeForSelectedCustomer;

                //if (!cache.TryGetValue<List<Ledger>>(key, out _ledgerEntries))
                //{
                _ledgerEntries = GetLedgerEntriesByCustomer(customerNo, (int)operationalProvider.AccountTypeForSelectedCustomer);
                ...
                //if (!cache.TryGetValue<List<LedgerLineItem>>(ledgerLineItemsKey, out _ledgerLineItems))
                //{
                var apiClient = new SkyBillApiClient(_companyName, cache);
        ...
        public OperationalBillingProvider(IMemoryCache cache, string customerNo, int accountTypeForSelectedCustomer, string companyName)
        {
            _companyName = companyName;
            _cache = cache;
            _ledgerEntries = GetLedgerEntriesByCustomer(customerNo, accountTypeForSelectedCustomer);
            var apiClient = new SkyBillApiClient(companyName, cache);
            _ledgerLineItems = apiClient.GetSalesInvoiceLinesByCustomer(customerNo);
        }
        ...
        private List<Ledger> GetLedgerEntriesByCustomer(string customerNumber, int accountType)
        {
            ...
            List<Ledger> ledgerEntries = new List<Ledger>();
            try
            {
                ledgerEntries = apiClient.GetLedgerEntriesByCustomer(customerNumber);
                int multiplier = 1;
        ...
        public Stream GetTenantConsumptionInvoice(string customerNo, string companyName, DateTime invoiceMonth, string templateFileName, Data.Company company, Data.Customer customer, bool includeBalances = false, string logoPATH = "")
        {
            SkyBillApiClient skyBillApiClient = new SkyBillApiClient(companyName, _cache);
            return skyBillApiClient.GetTenantConsumptionInvoice(customerNo, companyName, invoiceMonth, templateFileName, company, customer, includeBalances, logoPATH);
        }
